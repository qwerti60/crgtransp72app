...
class outputobz extends StatelessWidget {
  ...
  const outputobz({super.key, required this.nameImg});
...
//bool? isLiked = false;
  ...
  Future<bool> checkLike(String idUser, String id, int bd) async {
    //   final response = await http.get(Uri.parse(
    //     'http://yourdomain.com/c?idusers=$idUser&id=$id&bd=$bd'));
    final response = await http.get(
      Uri.parse(Config.baseUrl).replace(
        path: '/api/check_like.php',
        queryParameters: {
          'idusers': idUser,
          'id': id,
          'bd': bd
...
// В цикле forEach
                    ...
                    Text(
                      ...
                      style: const TextStyle(
                        fontWeight: FontWeight.bold,
                        color: Colors.black, // Установка цвета текста в черный
                      ),
                    );
                    print('В этом разделе нет объявлений');
                  }
